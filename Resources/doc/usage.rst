...
When you dump the routes from a command, the Request Context (e.g. host and base_url) are not known. To generate correct routes for each environment with console commands, specify the environment in your command and add the following (see `How to Generate URLs from the Console`):
